...
# 在100个点里面均匀取值选取20个样本点用来拟合半变异函数
    ...
    d_sv, sv = ph.genSemivar(xyzs_np, D_MAX, N_SEMIVAR, len(
        n)*len(n))  # 根据原始数据，D_MAX最大距离，N_SEMIVAR个数限制，
...
# 神经网络克里金（单线程）
    for i in range(0,len(y_valid)):
        for j in range(len(x_valid)):
            ...
            v, dnn_ok_error = ph.dnn_ok_v(
                [x, y, z, x_valid[j], y_valid[i], model, error_threshold, i, j, dis_num])
